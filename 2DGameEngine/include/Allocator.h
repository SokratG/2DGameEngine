...
namespace Utils {
	...
	const size_type CalcPadding(const size_type baseAddress, const size_type alignment) {
		const size_type multiplier = (baseAddress / alignment) + 1;
		const size_type alignedAddress = multiplier * alignment;
...
namespace TestAllocator {
	...
	protected:
		...
		AAllocator(const size_type totalSize) : totalAllocSize(totalSize), currentSize(0u), peakSize(0u) {}

		virtual ~AAllocator() { totalAllocSize = 0u; };

		virtual void_pointer allocate(const size_type size, const size_type alignment = 0u) = 0;

		virtual void deallocate(void_pointer ptr) = 0;
	...
	{
	public:
		using size_type = typename AAllocator::size_type;
		using void_pointer = typename AAllocator::void_pointer;
	...
	private:
		void_pointer _start_ptr;
		size_type _offset;

	public:
		LinearAllocator(const size_type totalSize) : AAllocator(totalSize), _start_ptr(nullptr), _offset(0u) {}

		virtual ~LinearAllocator() {
			::operator delete(_start_ptr);
			_start_ptr = nullptr;
		}

		virtual void_pointer allocate(const size_type alloc_size, const size_type alignment = 0) override;

		virtual void deallocate(void_pointer ptr) override {
			static_assert(true, "use only reset() method");
		}

		virtual void init() override {
			if (_start_ptr != nullptr)
				::operator delete(_start_ptr);
			_start_ptr = ::operator new(sizeof(void_pointer));
		}
	...
	public:
		LinearAllocator(const LinearAllocator& la) = delete;

	};


	template<typename T>
	typename LinearAllocator<T>::void_pointer LinearAllocator<T>::allocate(const size_type alloc_size, const size_type alignment) {
		size_type padding = 0u;
		size_type paddedAddress = 0u;
		const size_type _address = _start_ptr + _offset;

		if (alignment != 0u && _offset % alignment != 0u)
			padding = Utils::CalcPadding(_address, alignment);


		if (alloc_size + padding + _offset > totalAllocSize)  // exception
			return nullptr;

		_offset += padding;

		const size_type nextAddress = _address + padding;
		_offset += alloc_size;

		currentSize = _offset;
		peakSize = MAX(peakSize, currentSize);

		return reinterpret_cast<void_pointer>(nextAddress);
	}
